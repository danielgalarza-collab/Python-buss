...
estaComenzando = texto_a_modificar.startswith("este") # ACA PREGUNTA SI EL TEXTO ESTA COMENZANDO POR LA PALABRA QUE ELIJAMOS , DEVUELVE UN TRUE O FALSE
estaFinalizando = texto_a_modificar.endswith("MODIFICAR") # LO MISMO QUE ARRIBA PERO CUANDO FINALIZA
...
nombre = "Pedrito"
variable = "edad"
txt = f"La {variable} de {nombre} es de {num:.2f}" # Nos permite utilizar variables en texto y formatearlas , en el caso de num:.2f nos permite agregar 2 decimales 
print (txt)
resultado = f"El resultado de 69 x 75 es {69*75}"

# Escapes \ (backslash - barra invertida)
escape = "Mi país favorito es \"Lituania\" por que me dio la ciudadanía europea" # EL \\ SIRVE PARA CUANDO QUEREMOS USAR LAS COMILLAS DOBLES DENTRO DE LAS COMILLAS DOBLES 
directorio = "El directorio donde está almacenado mi mayor secreo es c:\\secreto\\archivo.txt" #CUANDO HAYA QUE PONER LA DIRECCION DE ALGO HAY QUE PONER DOBLE BARRA
